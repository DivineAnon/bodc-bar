					...
					<div class="col-12">
						<div class="card">
							<div class="card-header border-0">
								<h4 class="">Top 10 Problem UA EGI (<?php echo $egi;?>) <?php if($owner_name !='') echo ' Owner ('.$owner_name.')';?></h4>
							</div>
										...
										for ($i=0; $i < count($no_unit['no_unit']); $i++){
											...
											<tr>
												...
												if($no_unit['detail'][$i]['response'] == 'avail'){
													...
													$ua = $ua_loader = floatval($work)/(floatval($work) + floatval($stanby));
													;
													?>
													<td>
														<?php
												...
												?>
												<td>
													<a href="<?php echo base_url().'Ua/detail_ua_unit/'.$id_type_unit.'/'.$cn_unit;?>" class="btn btn-primary">
														Detail
													</a>
												</td>
											</tr>
...
<script>
	...
	function search(){
		...
		if(owner == ''){
			...
			window.location.href = '<?= base_url();?>Ua/search_data_ua_owner/'+'<?php echo $id_type_unit;?>'+'/'+'<?= $egi;?>'+'/'+owner;
		}
	}
</script>
</body>
